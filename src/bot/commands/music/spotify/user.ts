  ...
  public async exec(message: Message, { user }: { user: string }) {
    ...
    this.client.spotify.clientCredentialsGrant().then((data) => {
      ...
      this.client.spotify.getUser(user).then(async (res) => {
        ...
        const userPlaylists = await this.client.spotify.getUserPlaylists(user).then((resp) => {
          const playlists = resp.body.items.map((item) => {
          ...
          const playlistCount = pluralize('playlist', resp.body.total, true);
